...
import { Clock, Search, Shield, ChevronDown } from 'lucide-react';
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-lg hover:border-slate-700 transition-all"
          >
            <Shield size={14} className={user.role === 'admin' ? 'text-rose-400' : user.role === 'developer' ? 'text-violet-400' : 'text-slate-400'} />
          ...
          {isRoleMenuOpen && (
            <div className="absolute right-0 mt-2 w-40 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-50 overflow-hidden">
              <div className="p-2 border-b border-slate-800">
                <p className="text-[10px] font-bold text-slate-500 uppercase px-2 py-1">Switch Role</p>
              </div>
              {roles.map((role) => (
                <button
                  key={role}
                  ...
                  className={`w-full text-left px-4 py-2 text-xs font-medium transition-colors ${
                    user.role === role ? 'bg-violet-600/10 text-violet-400' : 'text-slate-400 hover:bg-slate-800'
                  }`}
                >
        ...
        <div className="w-8 h-8 bg-violet-600 rounded-full flex items-center justify-center text-white font-bold text-xs shadow-lg shadow-violet-900/20 overflow-hidden">
